...
const SOURCE_LABEL = 'Tracxn AI startups in Singapore (top public list)';
const SOURCE_DATE = '2026-08-07';
const SOURCE_URL = 'https://tracxn.com/d/artificial-intelligence/ai-startups-in-singapore-in-singapore/__PfOS3PCPjbVgvUNafWRvsLTB4R8VL6dUCQ5BZhsZZNc/companies#t-5-aethir';

const STARTUPS = [
  {
    name: 'Ai Palette',
    description: 'AI-powered platform for CPG innovation, trend discovery, and product development',
    city: 'Singapore',
    country: 'Singapore',
    foundedYear: 2018,
    stage: 'Acquired',
    status: 'Acquired',
    fundsRaisedMUsd: 12.7,
    website: 'https://tracxn.com/d/companies/ai-palette/__bshx6s_LmylnLUTJNcrPypAAKW-_knzj0rUoriF3ILk'
  },
  {
    name: 'CloudSEK',
    description: 'Provider of predictive attack path intelligence and digital risk monitoring',
    city: 'Singapore',
    country: 'Singapore',
    foundedYear: 2015,
    stage: 'Series B',
    status: null,
    fundsRaisedMUsd: 42.7,
    website: 'https://tracxn.com/d/companies/cloudsek-research/__Db3ID9fFfpAkM7Emu_FS5Zhd-Wq3ehEqc6aIyIwfrvs'
  },
  {
    name: 'Gero',
    description: 'Developer of physics based artificial intelligence models for drug discovery',
    city: 'Singapore',
    country: 'Singapore',
    foundedYear: 2018,
    stage: 'Series A',
    status: null,
    fundsRaisedMUsd: 25.2,
    website: 'https://tracxn.com/d/companies/gero/__gUEyveHHGLeBJ1I6eJ4dtim3JZdfZvZ7xDF-Td7gExk'
  },
  {
    name: 'Graas',
    description: 'AI-based analytics solutions for e-commerce businesses',
    city: 'Singapore',
    country: 'Singapore',
    foundedYear: 2022,
    stage: 'Series A',
    status: null,
    fundsRaisedMUsd: 49,
    website: 'https://tracxn.com/d/companies/graas/__I2i28CMlfSsawecQuG3kSAKS6ROdKssoIDHFpEFUYJw'
  },
  {
    name: 'Aethir',
    description: 'Cloud infrastructure for AI and gaming industry',
    city: 'Singapore',
    country: 'Singapore',
    foundedYear: 2021,
    stage: 'Seed',
    status: null,
    fundsRaisedMUsd: 9,
    website: 'https://tracxn.com/d/companies/aethir/__JXFumZl9z4l4iMTjD2Q6CuBz4vD9LyvlJcpX-qDjqzk'
  }
];
...
async function main() {
  ...
  try {
    ...
    for (const item of STARTUPS) {
      ...
      if (!existing) {
        if (APPLY) {
          const description = `${item.description}. ${SOURCE_LABEL} (${SOURCE_DATE}). Stage: ${item.stage}. Funds raised reported: ${item.fundsRaisedMUsd} MUSD. Source: ${SOURCE_URL}`;
          await run(
            db,
      ...
      if (!existingDescription && item.description) {
        updates.push('description = ?');
        params.push(`${item.description}. ${SOURCE_LABEL} (${SOURCE_DATE}). Stage: ${item.stage}. Funds raised reported: ${item.fundsRaisedMUsd} MUSD. Source: ${SOURCE_URL}`);
